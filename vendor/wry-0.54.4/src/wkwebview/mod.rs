...
pub fn platform_webview_version() -> Result<String> {
  #[cfg(target_os = "ios")]
  {
    // iOS 上 WKWebView 属于系统组件。
    // 这里如果继续走 NSBundle::bundleWithIdentifier("com.apple.WebKit")
    // 在当前模拟器/Xcode 组合下会直接触发原生崩溃，
    // 而 tauri-runtime-wry 这里只是用 is_ok() 判断 WebView 运行时是否可用。
