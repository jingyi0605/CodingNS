...
describe("ReleasePanel", () => {
  ...
  it("展示 GitHub Release 的 tag 和说明，并支持打开更新页面", async () => {
    ...
    expect(await screen.findByText("修掉旧的 manifest 检查逻辑")).toBeInTheDocument();
    expect(screen.getByText(/v0\.2\.0/)).toBeInTheDocument();
    expect(screen.getByText(t("settings.releaseSignatureMissing"))).toBeInTheDocument();
    expect(screen.getByRole("button", { name: t("settings.releaseInstallNow") })).toBeDisabled();
