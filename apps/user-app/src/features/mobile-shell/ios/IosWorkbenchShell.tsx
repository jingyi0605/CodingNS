...
export function IosWorkbenchShell({
  ...
  const tabbarContent = (
    <nav ref={tabbarRef} className="ios-workbench-tabbar" aria-label={t("shell.title")}>
      {navItems.map((item) => (
        <button
          key={item.key}
          type="button"
...
function AssistantIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.85">
      ...
      <circle cx="10" cy="12" r=".95" fill="currentColor" stroke="none" />
      <circle cx="14" cy="12" r=".95" fill="currentColor" stroke="none" />
