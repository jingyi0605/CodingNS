...
export class ButlerControlSessionService {
  ...
  async startSession(
    ...
    const profile = this.butlerProfileService.ensureInitialized();
    const promptContext = this.butlerContextAggregator.resolvePromptContext(userId, input.content ?? null);
    const workspace = this.prepareWorkspace(profile, promptContext);
    const current = this.butlerControlSessionRepository.findLatestByProvider(profile.providerId);
    ...
    const content = normalizeControlContent(
      input.content,
      "请先以代码管家的身份介绍当前职责范围，并等待用户安排工作。"
    );
  ...
  async resumeCurrentSession(
    ...
    const promptContext = this.butlerContextAggregator.resolvePromptContext(userId, null);
    this.syncWorkspaceContext(profile, promptContext);
  ...
  async sendMessage(
    ...
    const promptContext = this.butlerContextAggregator.resolvePromptContext(userId, content);
    this.syncWorkspaceContext(profile, promptContext);
  ...
  private prepareWorkspace(profile: ButlerProfile, promptContext: ButlerPromptContext) {
    this.syncWorkspaceContext(profile, promptContext);
    return this.workspaceService.importWorkspace(profile.workspacePath, "代码管家");
  }

  private syncWorkspaceContext(profile: ButlerProfile, promptContext: ButlerPromptContext): void {
    fs.mkdirSync(profile.workspacePath, { recursive: true });
    ensureButlerWorkspaceIsolation(profile.workspacePath);
    writeInstructionFiles(profile, promptContext);
    syncCodexInstructionConfig(profile, this.codexHomeDir);
  }
...
function writeInstructionFiles(profile: ButlerProfile, promptContext: ButlerPromptContext): void {
  ...
  writeFileIfChanged(path.join(profile.workspacePath, "BUTLER_API.md"), buildApiGuideContent());
}

function syncCodexInstructionConfig(profile: ButlerProfile, codexHomeDir: string | null): void {
...
function buildApiGuideContent(): string {
  return `# 代码管家内部补查接口
...
2. 用户问全局情况时，补查 \`GET /api/butler/overview\`。
3. 用户明确追问某个项目时，补查 \`GET /api/butler/projects/:projectId/context\`。
4. 仍然不够时，再按既有 butler 细节接口查询项目、会话、记忆、巡视、验证对象。
