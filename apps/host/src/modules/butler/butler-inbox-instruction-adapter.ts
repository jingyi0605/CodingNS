...
export function buildButlerInboxAnalysisInstruction(input: {
  ...
  const prompt = [
    ...
    "2. 先确认 CLI 默认认证入口可用；Butler 工作区里优先直接使用 CLI 对 `BUTLER_AUTH.json` 的自动发现，不要把手工 export 环境变量当成每轮固定动作。",
    "3. 先确认 CLI 可用：至少执行一次 `codingns assistant capabilities list`。",
    `4. 必须读取当前项目详情：至少执行一次 \`codingns assistant projects get ${input.project.id}\`。`,
