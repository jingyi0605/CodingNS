...
export interface ExportV2BuildOptions {
  dirtyScope?: DirtyScope;
  light?: boolean;
}

export interface ExportV2BuildResult {
  outputDir: string;
  manifestPath: string;
 ...
 */
export class ExportV2Builder {
  constructor(private readonly config: RuntimeConfig) {}

  build(options: ExportV2BuildOptions = {}): ExportV2BuildResult {
    ...
    ensureDir(this.config.exportV2Dir);
    const metaDir = path.join(this.config.exportV2Dir, "meta");
    const detailDir = path.join(this.config.exportV2Dir, "detail");
    const tagDir = path.join(this.config.exportV2Dir, "tags");
    const relationDir = path.join(this.config.exportV2Dir, "relations");
    const tempDir = path.join(this.config.exportV2Dir, ".tmp");
    ...
    const previousManifestPath = path.join(this.config.exportV2Dir, "manifest.json");
    ...
    const flushMetaShard = (): void => {
      ...
      if (fullBuild || shardDirectories.some(directory => dirtyDirectories.has(directory))) {
        const absolutePath = path.join(this.config.exportV2Dir, relativePath);
    ...
    for (const batch of repository.iterateExportDocumentRecords(2000)) {
      for (const document of batch) {
        ...
        if (fullBuild || changedPaths.has(document.path) || dirtyDirectories.has(directory)) {
          const absoluteDetailPath = path.join(this.config.exportV2Dir, detailRelativePath);
    ...
    let currentPostings: Array<{ tag_path: string; document_count: number; documents: Array<{ document_id: string; path: string; title: string; derived: boolean }> }> = [];
    ...
    const flushCurrentRoot = (): void => {
      ...
      if (fullBuild || dirtyRoot) {
        ...
        const absolutePostingPath = path.join(this.config.exportV2Dir, postingPath);
        writeJson(absolutePostingPath, {
          version: 2,
        ...
        filesWritten.push(absolutePostingPath);
      }
    ...
    for (const fileName of relationTempFiles) {
      ...
      const rawRecords = readNdjson<RelationPair>(path.join(relationTempDir, fileName));
      ...
      if (fullBuild || dirtyRelationIds.has(documentId)) {
        const absolutePath = path.join(this.config.exportV2Dir, "relations", `${documentId}.json`);
        writeJson(absolutePath, {
          version: 2,
          shard_type: "relation",
          exported_at: exportedAt,
    ...
    const statusPath = path.join(this.config.exportV2Dir, "status.json");
    const taxonomyPath = path.join(this.config.exportV2Dir, "taxonomy.json");
    const relationsPath = path.join(this.config.exportV2Dir, "relations.json");
    const bootstrapPath = path.join(this.config.exportV2Dir, "bootstrap.json");

    const searchIndexResult = lightBuild
      ? { bucketCount: 0, filesWritten: [] as string[], manifestPath: path.join(this.config.exportV2Dir, "search", "manifest.json") }
    ...
    const manifestPath = path.join(this.config.exportV2Dir, "manifest.json");
    ...
    if (previousManifest) {
      ...
      for (const shard of previousManifest.meta_shards ?? []) {
        ...
        if (
          ...
          const absolutePath = path.join(this.config.exportV2Dir, shard.path);
    ...
    return {
      outputDir: this.config.exportV2Dir,
      manifestPath,
