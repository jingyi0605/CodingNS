...
export type ConptyRuntimeType =
  | "conpty-powershell"
  | "conpty-cmd"
  | "conpty-git-bash";

export interface RuntimeScriptLaunch {
  command: string;
  args: string[];
  cwd: string;
}

export function isConptyRuntimeType(value: string): value is ConptyRuntimeType {
  return (
...
export function assertConptySupported(): void {
...
export function buildConptyPipeName(sessionKey: string): string {
...
export function resolveRuntimeScriptLaunch(scriptBaseName: string): RuntimeScriptLaunch {
...
export function readRequiredCliArg(
...
export function isPersistentTerminalRuntime(
  runtimeType: TerminalRuntimeType
): boolean {
  return runtimeType !== "embedded-pty";
}
