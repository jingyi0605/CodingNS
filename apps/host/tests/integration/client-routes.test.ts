...
`,
  ...
  it("可以创建并执行真实 Chrome 浏览器任务，产生截图和 DOM 产物", async () => {
    const fixture = createEmptyFixture();
    activeFixtures.push(fixture);
    ...
    expect(taskDetail.task.status).toBe("succeeded");
    expect(taskDetail.artifacts.length).toBeGreaterThanOrEqual(2);
  ...
  it("可以创建 CDP attach 浏览器 Profile 并执行任务", async () => {
    ...
    const attachResponse = await hosted.app.inject({
      method: "POST",
      url: "/api/office/browser/cdp/attach",
      headers,
      payload: {
        engine: "chrome",
    ...
    const profileId = attachResponse.json().id as string;
  ...
  it("可以创建并执行 SSH 运维任务，产出 stdout/stderr 与回执", async () => {
    ...
    spawnMock.mockImplementation((command) => {
      if (command === "ssh") {
        return createMockChildProcess("Filesystem      Size Used Avail Use% Mounted on\n/dev/disk1  100G  60G   40G  60% /\n");
      }
    ...
    expect(createTaskResponse.statusCode).toBe(200);
    const sshTaskId = createTaskResponse.json().task.id as string;

    const executeResponse = await hosted.app.inject({
      method: "POST",
      url: `/api/office/ops/ssh/tasks/${sshTaskId}/execute`,
      headers
    });

    expect(executeResponse.statusCode).toBe(200);

    const taskDetail = await waitForOfficeTask(hosted, tokens.accessToken, sshTaskId);
    expect(taskDetail.task.status).toBe("succeeded");
    expect(taskDetail.receipts[0]?.receiptType).toBe("ssh_execution");
    expect(taskDetail.receipts[0]?.payloadJson).toContain("\"host\":\"10.0.0.8\"");
    expect(taskDetail.artifacts.some((item) => item.storagePath?.endsWith("stdout.log"))).toBe(true);
    expect(spawnMock).toHaveBeenCalledWith(
      "ssh",
      expect.arrayContaining([
        "-p",
