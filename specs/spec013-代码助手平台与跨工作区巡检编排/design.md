...
- 把“代码助手”做成平台一等能力，而不是 provider 包装壳
...
- 助手初始化、控制会话、聚合对话和前端工作台边界拆到 `spec013.1`

## 2. 核心思路

### 2.1 为什么“代码助手”必须是平台层，而不是 provider 层
...
用户真正要的是“统一的代码助手认知”，不是“所有 provider 都恰好读同一个文件名”。
...
- `spec013` 保留平台事实层和执行层
- `spec013.1` 承接助手初始化、控制会话、聚合上下文、聊天 API 和工作台 UI
...
用户真正可见的助手控制会话、聊天入口和工作台页面，统一在 `spec013.1` 实现。
