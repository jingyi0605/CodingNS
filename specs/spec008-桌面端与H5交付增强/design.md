...
- `spec008` 负责桌面壳交付、连接、升级、配置和桌面/H5 共用运行时边界
- `spec008.1` 负责桌面端工作台多窗口
- `spec008.2` 负责桌面端与移动端模态框统一模板、尺寸档位和迁移收口
- 如果问题的核心是“一个桌面工作台视图要不要拆成独立窗口”，归 `spec008.1`
- 如果问题的核心是“弹窗和 sheet 应该如何统一尺寸、布局、操作区和控件模板”，归 `spec008.2`
- 如果问题的核心是“桌面壳怎么安装、更新、连接、配置”，归 `spec008`
