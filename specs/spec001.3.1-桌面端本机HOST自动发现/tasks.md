...
- [x] 1.1 定义自动发现 HOST 的运行时数据结构
  ...
  - 怎么验证：
    ...
    - 文档与代码结构走查
  - 验证结果：
    - 已新增 `discoveredHosts[]`、`activeDiscoveredHostId`、扫描状态和稳定发现键，并保持为运行时数据，不写回正式 HOST 配置
...
- [x] 1.2 定义桌面壳进程扫描桥接口
  ...
  - 怎么验证：
    ...
    - 桌面桥调用测试
  - 验证结果：
    - 已补 `scan_local_hosts` Tauri 命令、桌面桥 DTO 和 `platform-adapter` 调用入口

---

## 阶段 2：实现扫描、探活、去重

- [x] 2.1 Windows、macOS 本机进程扫描落地
  - 状态：DONE
  ...
  - 怎么验证：
    ...
    - 桌面端手动样例验证
  - 验证结果：
    - 已实现 `ps` / `PowerShell` 进程枚举和 `codingns start` 参数解析，`cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml host_discovery -- --nocapture` 通过

- [x] 2.2 在客户端建立自动发现运行时 store
  - 状态：DONE
  ...
  - 怎么验证：
    ...
    - 探活回归测试
  - 验证结果：
    - 已新增本机 HOST 自动发现 store，接入后台扫描、探活、冷却窗口和运行时状态更新，相关单测通过

- [x] 2.3 把自动发现 HOST 与手动 HOST 按地址去重
  - 状态：DONE
  ...
  - 怎么验证：
    ...
    - HOST 列表回归测试
  - 验证结果：
    - 已实现自动发现内部按地址去重，以及自动发现与手动 HOST 的地址去重，手动 HOST 优先展示

---

## 阶段 3：把 UI 和凭据接上

- [x] 3.1 在 HOST 列表里增加“自动发现”分类
  - 状态：DONE
  ...
  - 怎么验证：
    ...
    - 桌面端手动验收
  - 验证结果：
    - 已在桌面端 HOST 切换菜单中加入“手动保存 / 自动发现”分组，并完成组件测试与实际打包验证

- [x] 3.2 为自动发现 HOST 接入用户名、密码本地保存
  - 状态：DONE
  ...
  - 怎么验证：
    ...
    - HOST 列表回填测试
  - 验证结果：
    - 已让自动发现 HOST 使用稳定发现键复用本地 remember-password 和 HOST 级会话存储，再次出现时可按同一键回填

---

## 阶段 4：收口体验和回归

- [x] 4.1 扫描失败、无结果、冷却窗口体验收口
  - 状态：DONE
  ...
  - 怎么验证：
    ...
    - 手动回归
  - 验证结果：
    - 已补扫描中、扫描失败和冷却复用状态，相关 store/组件测试通过，`bash scripts/build-desktop.sh` 成功产出 macOS 客户端
