...
- 3.2 继续推进中：
  - 已修正 Codex 新建实时会话时错误在 `thread.id` 为空时伪造 UUID 作为 `providerSessionId` 的问题。
  - 运行时现在直接等待 Codex SDK 的 `thread.started.thread_id`，再用这枚真实 thread id 绑定 host 与 `.codex` 原生会话，不再靠本地 sqlite 猜测映射。
- 4.2 继续推进中：
  - 已补前端 draft -> 真实会话的即时切换。
  - `start-live` 成功后前端会先本地插入并选中真实 host session，再后台刷新导航，并补齐 workspace 映射，不再阻塞在 draft 页面等待慢刷新。
...
- [ ] 5.2 最终检查点：Spec 收口
  ...
  - 对应设计：`design.md` 全文
> 2026-03-24 补记：
> - 3.2 / 4.2 继续推进，已收紧前端“实时消息”和“历史权威消息”的边界。
> - `pending://` / `synthetic://` 用户消息在收到权威历史消息后会按内容和时间窗替换，避免同一句用户消息重复显示。
> - 聊天页自动滚动已改为跟踪最后一条消息的内容签名，AI 流式续写同一条消息时也会继续滚动到底部。
> - 聊天页用户消息布局已补成稳定靠右显示。
> - 已补回归测试：`session-runtime-machine.test.ts` 和 `MessageTimeline.test.tsx`。
> 2026-03-24 补记：
> - 4.2 继续推进，已把发送态和运行态统一收口到 Composer。
> - 用户提交消息后，输入框现在会立刻清空，不再等 Codex 开始回流后才清空。
> - Composer 已增加显式交互态提示，发送中显示“发送中”，运行中显示“运行中”。
> - 发送按钮和运行态按钮已改成互斥显示，交互态下不再同时出现发送和停止两个按钮。
> - 已补 `ComposerPanel.test.tsx` 回归测试，并通过前端类型检查与相关会话测试。
> 2026-03-24 补记：
> - 4.2 继续推进，已修复运行态收尾链路。
> - 前端收到 `session.delta/backfill` 时，不再无条件把已完成会话覆写回 `running`。
> - 最后一波消息到达后会主动复查 `/runtime`，用于兜底错过完成事件的情况，避免发送按钮长期不可用。
> - 已补 `session-runtime-store.test.ts` 回归测试，并通过前端类型检查与相关会话测试。
