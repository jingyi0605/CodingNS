...
9. `costUsd` 必须由既有 `ProviderSessionStats` 的同一次读取和折叠产生。系统不得增加 `readCost`、独立成本 API、后台轮询、第二份日志扫描或平行账本。
10. OpenCode 已记录的原生成本优先于任何目录价格。其他 Provider 仅在已确认直连收费路由且价格表覆盖全部 token 桶时，才输出目录估算费用。
11. 目录价格必须来自版本固定的本地价格表；运行时不得访问外部价格 URL。机器可读目录可作为受控更新输入，但不能成为运行时依赖。
...
- 不新增数据库表、后台任务、轮询器、独立订阅、独立成本服务或每轮网络价格查询。为新会话在既有 `session_bindings` 保存收费策略版本属于会话元数据，不是新账本。
