...
- 已明确 `spec013` 只保留事实层和执行编排层，不再承接管家聊天、控制会话和工作台 UI。
- 已明确 `ButlerProfile`、`ButlerControlSession`、`ButlerContextSnapshot`、Butler Chat API、控制动作 API、前端管家工作台统一下沉到 `spec013.1`。
- 后续如果继续做“管家如何和用户对话”，一律去 `spec013.1`，不要再往 `spec013` 里塞。
