...
### 2.6.1 后续新任务必须按统一接入规范实现

这次已经把最小骨架落下来了，后面再加后台任务时，不允许回到“每个模块自己搞一套”的老路。

强制规则：

1. 新任务先判断是否真的属于“跨请求、可延后、需要去重和观测”的后台任务
2. 属于的话，必须走统一 `TaskManager`
3. 必须先定稳定的 `taskType`
4. 必须把 `key` 设计成同一资源能真正去重
5. 必须明确执行位点：`host_background`、`helper_process`、`external_process`
6. 请求入口必须先走缓存，再决定是否 enqueue
7. 不允许再引入新的散装 `inflight`、私有重试队列、私有后台定时器状态

具体接入步骤、模板代码、测试清单和反模式，统一看：
