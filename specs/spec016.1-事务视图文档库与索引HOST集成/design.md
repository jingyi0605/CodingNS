...
- `requirements.md` 需求 7.1：事务视图助手初始化逻辑升级
- `requirements.md` 需求 7.2：前端样式改造范围收敛

### 1.3 技术约束

- 前端修改范围锁定在 `apps/user-app`
- 只要新增或修改前端页面、区块、按钮、列表、空态，必须遵守现有前端设计规范；如涉及模态框，必须同时遵守模态框规范
...
7. 前端开始读取文档库快照
8. 如果此时还没有真实文档对象，右侧详情和助手进入“先完成绑定或先选择文档”的初始化态
9. 如果已经存在默认文档对象，右侧详情和助手直接切到真实文档语境
...
1. 文档库绑定入口
2. 文档库空态 / 错误态 / 刷新态
3. 左侧真实收藏 / 标签列表和中间真实文档列表
4. 右侧真实详情与助手初始化态

不做的东西：

- 与文档库主链路无关的整页重画
- 借这次顺手把待办、自动化、插件分区全部重新设计
- 纯审美式调整但不影响主链路可用性的样式返工

### 3.1.2 助手初始化逻辑升级边界

这次要改的不是 Butler 底层运行时，而是事务视图右侧助手的初始化语境：

1. 先判断当前工作区有没有绑定文档库
2. 没绑定时进入明确的空上下文初始化态
3. 已绑定但还没选中文档时进入“等待选择真实对象”的初始化态
4. 一旦存在默认文档或用户选中文档，就把语境切到真实文档对象

也就是说，这次是“初始化语境升级”，不是“助手引擎重写”。
