...
- [ ] 1.1 增加 `cc-switch` 命令路径发现和可用性判断
  ...
  - 主要改哪些文件：
    ...
    - `apps/host/src/shared/utils/command-availability.ts`
    - `apps/host/src/modules/model-switch/*`
  - 这一步先不做什么：不执行切换，不解析预设。
  - 怎么验证：
    - Host 单元测试
    - 本机路径手工验证

- [ ] 1.2 建立 `cc-switch` 结构化读取适配层
  - 状态：TODO
  ...
  - 主要改哪些文件：
    ...
    - 相关测试
  - 这一步先不做什么：不暴露敏感字段，不解析表格输出。
  - 怎么验证：
    - 适配层单元测试
    - 去敏感化字段检查

- [ ] 1.3 增加模型管理 API 和切换接口
  - 状态：TODO
...
- [ ] 2.2 新增桌面端 `ModelManagementPanel`
  - 状态：TODO
...
- [ ] 3.1 切换成功后刷新相关 provider capability 显示
  - 状态：TODO
  - 这一步到底做什么：保证设置页切换成功后，相关“跟随 CLI 默认模型”显示能尽快更新。
  - 做完以后能看到什么结果：用户切完以后不是还盯着旧值发愣。
  - 依赖什么：2.3
  - 主要改哪些文件：
    - `apps/user-app/src/features/conversation/api/*`
