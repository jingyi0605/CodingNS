...
- 已补充 `spec001.1-账户偏好入库与跨客户端同步` 的子规格边界
- 已明确账户级偏好入库和客户端配置分层不再塞回 `spec001` 主文档里混做，而是由 `spec001.1` 单独承担

## 2026-04-12 进展补记

- 已启动 `spec001.2-后端任务调度与主线程压力治理`
- 已明确请求链路拆分、后台任务治理、主线程压力量化不再塞回 `spec001` 主文档里混做，而是由 `spec001.2` 单独承担

## 2026-04-16 进展补记

- 已启动 `spec001.2.1-读写刷新与后台任务统一规则`
- 已明确读、写、刷新、watcher、散装 `inflight/timer` 的统一规则不再塞回 `spec001` 主文档里混做，而是由 `spec001.2.1` 单独承担

## 2026-04-14 进展补记

- 已启动 `spec001.3-多HOST接入与跨端切换`
- 已明确多 HOST Profile、激活 HOST 切换、HOST 级登录态保存不再塞回 `spec001` 主文档里混做，而是由 `spec001.3` 单独承担

## 2026-04-16 进展补记（追加）

- 已启动 `spec001.3.1-桌面端本机HOST自动发现`
- 已明确桌面端本机 `codingns` 进程自动发现、自动发现分组和自动发现与手动 HOST 去重不再塞回 `spec001` 主文档里混做，而是由 `spec001.3.1` 单独承担

## 2026-06-09 进展补记

- 已启动 `spec001.3.2-当前HOST代理访问其他HOST仓库`
- 已明确当前 HOST 代理访问其他 HOST 仓库、同版本检查、目标 HOST 登录态代管和 `hostId + workspaceId` 工作区引用不再塞回 `spec001.3` 主文档里混做，而是由 `spec001.3.2` 单独承担

## 2026-06-16 进展补记

- 已启动 `spec001.3.3-HOST与PEERHOST资源作用域统一与切换收口`
- 已明确会话、终端、文件、Git、调试这些跨 HOST 资源的归属模型、URL 生成规则和切换时的连接清理不再塞回 `spec001.3.2` 主文档里混做，而是由 `spec001.3.3` 单独承担

## 2026-04-14 进展补记（追加）

- 已启动 `spec001.4-Tailscale接入与实例级远程访问`
- 已明确实例级 Tailscale 配置、设置页动态启停、tailnet 地址展示和未初始化暴露限制不再塞回 `spec001` 主文档里混做，而是由 `spec001.4` 单独承担

## 2026-04-15 进展补记

- 已启动 `spec001.6-客户端与服务端统一更新机制`
- 已明确桌面端更新、Android `APK` 直装、服务端全局 npm 包升级不再塞回 `spec001` 主文档里混做，而是由 `spec001.6` 单独承担

## 2026-06-12 进展补记

- 已启动 `spec001.12-更新通道与预览版本更新体验`
- 已明确稳定版 / 开发版更新通道、开发版确认弹窗、`vX.Y.Z-beta.N` 预发布规则和更新日志展示不再塞回 `spec001.6` 主文档里混做，而是由 `spec001.12` 单独承担

## 2026-04-15 进展补记（追加）

- 已启动 `spec001.7-设置页模型快速切换与CC-SWITCH接入`
- 已明确设置页模型管理、`cc-switch` 预设切换适配和四个 CLI 的统一状态展示不再塞回 `spec001` 主文档里混做，而是由 `spec001.7` 单独承担

## 2026-04-18 进展补记

- 已启动 `spec001.8-登录设备管理与主设备控制`
- 已明确登录设备管理、最近 10 条登录记录、主设备显式设置和主设备退出其他设备不再塞回 `spec001` 主文档里混做，而是由 `spec001.8` 单独承担

## 2026-04-19 进展补记

- 已启动 `spec001.9-公共隧道服务接入与端到端加密远程访问`
- 已明确公共隧道服务、主仓库与独立子仓库分工、端到端加密传输、三级域名入口边界和流量计费不再塞回 `spec001` 主文档里混做，而是由 `spec001.9` 单独承担

## 2026-04-21 进展补记

- 已启动 `spec001.9.4-公共隧道盲中继收口与可信接入端直连`
- 已明确 `relay-edge` 的公网明文业务代理不能继续留在主方案里，盲中继收口、可信 H5 / 桌面客户端直连和 Channel 端简化链路不再塞回 `spec001` 主文档里混做，而是由 `spec001.9.4` 单独承担
