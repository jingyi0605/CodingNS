...
- 稳定版 / 开发版更新通道、开发版确认和预览版本发布规则由 `spec001.12-更新通道与预览版本更新体验` 继续展开
- 设置页里基于 `cc-switch` 的模型预设快速切换由 `spec001.7-设置页模型快速切换与CC-SWITCH接入` 继续展开
- 登录设备管理、最近 10 条登录记录和主设备控制由 `spec001.8-登录设备管理与主设备控制` 继续展开
- 公共隧道服务接入、主仓库与独立子仓库分工、端到端加密传输和流量计费边界由 `spec001.9-公共隧道服务接入与端到端加密远程访问` 继续展开
- 公共隧道控制台站点、正式数据库、真实邮件、真实支付联调和生产化验收由 `spec001.9.1-公共隧道服务二阶段收口与生产化验收` 继续展开
- 公共隧道管理员后台、角色模型、管理员鉴权和只读管理视图由 `spec001.9.2-公共隧道管理员只读后台与权限通路` 继续展开
- 公共隧道流量优化、局域网自动直连和远程访问高频读链路减量由 `spec001.9.3-公共隧道流量优化与局域网自动直连` 继续展开
- 公共隧道盲中继收口、可信 H5 / 桌面客户端直连和 Channel 端简化通信流程由 `spec001.9.4-公共隧道盲中继收口与可信接入端直连` 继续展开
