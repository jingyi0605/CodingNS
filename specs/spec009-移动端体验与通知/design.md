...
`spec009` 不再单独承担“把 PC 三栏工作台拆成移动端页面结构”的细节设计。
...
| 文档 | 负责什么 | 明确不负责什么 |
| --- | --- | --- |
| `spec009` | 三层架构、平台适配层、系统能力、通知、原生壳、移动端能力总盘子 | 不展开每一个页面入口如何从三栏迁到移动端 |
| `spec009.1` | 当前 PC 三栏工作台的移动端导航、页面职责、动作迁移矩阵、宽度等级策略 | 不展开通知、分享、权限、生物识别等系统能力细节 |

要求：

- 以后凡是讨论“工作区 / 会话 / 工具 / 设置”一级结构、会话页怎么沉浸、右侧信息栏怎么拆页，统一引用 `spec009.1`
- 以后凡是讨论 `core / platform adapter / platform ui`、推送、权限、生命周期、壳层接入，统一回到 `spec009`

### 1.6 与 spec009.2 的边界

`spec009.2` 是 `spec009.1` 之后继续往下拆的实现级子问题，专门处理这次已经明确的入口重排。

它负责：

- 对话页左侧页承载文件 / Git
- 底部导航中“终端 / 助手 / 工具”的职责调整
- 助手页左右侧页和右侧信息栏三标签
- 旧工具路由的兼容迁移
...
- 以后凡是讨论“文件 / Git 为什么不再挂工具页”、“助手为什么升一级”、“butler 页面左右侧页怎么拆”，统一引用 `spec009.2`
- 以后凡是讨论推送、权限、生物识别、原生桥接，仍然统一回到 `spec009`
...
详细的“PC 三栏如何迁成移动端导航和页面结构”，已经下沉到 `spec009.1`。

本节只保留总约束，不重复写一份更啰嗦的页面迁移稿。
