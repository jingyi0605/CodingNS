...
- 已拆出 `spec009.1-移动端工作台导航与信息架构重构`
- `spec009.1` 专门负责“当前 PC 三栏工作台如何改成 H5/iOS/Android 移动端页面结构”
- `spec009` 后续继续保留总架构、平台适配层、系统能力、通知与原生壳的主任务

## 2026-04-15 补记

- 已拆出 `spec009.2-移动端对话主入口与助手侧栏重构`
- `spec009.2` 专门负责“对话页重新成为主入口、文件 / Git 收回对话侧页、底部终端 / 助手职责调整、助手页左右侧页补齐”
- `spec009` 继续只保留总架构、系统能力和平台适配层的主任务，不再展开这轮入口重排细节
...
- [ ] 1.3 建立移动端应用壳和平台导航容器
  ...
  - 先依赖什么：1.2
  - 详细页面结构依赖什么：
    - `spec009.1-移动端工作台导航与信息架构重构/design.md`
  - 主要改哪些文件：
