...
- 提供搜索、最近打开、预览、基础编辑能力
- 提供会话文件管理挂载能力，并与 `spec003` 会话运行时对接
...
- `requirements.md` 需求 5：文件管理挂载与 spec003 对接
- `requirements.md` 需求 6：文件管理不伪造会话消息真相
...
3. 会话上下文层：负责文件管理挂载、解绑、查询，并把结果提供给 `spec003`。
...
| `file-context-service` | 会话文件管理挂载、解绑、查询 | sessionId、filePath、range/hash | 绑定记录列表 |
...
- `FileContextController`：会话文件管理挂载与解绑入口
...
### 6.2 属性 2：文件管理不改变会话消息真相

*对于任何* 会话文件管理挂载操作，系统都应该满足：只新增/更新绑定元数据，不产生第二份会话原始消息源。
